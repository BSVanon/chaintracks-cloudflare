        ...
        Err(e) => {
            ...
            if upstream_url.is_none() {
                if let Ok(Some(tip)) = catch_up_from_bulk_cdn(&db, &chain, our_height).await {
                    ...
                    // Same finalize as the normal catch-up path (best-effort —
                    // this is a degraded tick): sweep C3 dual-active debris and
                    // record progress. Cumulative-work repair is skipped here;
                    // it targets the live-tip window, which a deep catch-up has
                    // not reached, and it runs on the next healthy tick.
                    let _ = sweep_dual_active(&db).await;
                    let _ = update_sync_state(&db, tip).await;
                }
            }
            return Ok(());
        }
